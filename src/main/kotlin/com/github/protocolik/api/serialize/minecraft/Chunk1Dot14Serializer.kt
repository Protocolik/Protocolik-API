...
object Chunk1Dot14Serializer : Serializer<Chunk> {
    override fun read(byteBuf: ByteBuf): Chunk {
        ...
        val sections = Array(16) {
            if (usedSections[it]) { // Section not set
                ...
                val section: ChunkSection = byteBuf.readChunkSection(ProtocolVersion.RELEASE_1_14)
    ...
    override fun write(byteBuf: ByteBuf, value: Chunk) {
        ...
        try {
            for (i in 0..15) {
                ...
                buf.writeChunkSection(section, ProtocolVersion.RELEASE_1_14)
            }
            buf.readerIndex(0)
