...
class Chunk1Dot9Dot4Serializer(override val param: Dimension) : PartialSerializer<Chunk, Dimension> {
    override fun read(byteBuf: ByteBuf, param: Dimension): Chunk {
        ...
        val sections = Array(16) {
            if (usedSections[it]) { // Section not set
                val section: ChunkSection = byteBuf.readChunkSection(ProtocolVersion.RELEASE_1_9_4)
    ...
    override fun write(byteBuf: ByteBuf, param: Dimension, value: Chunk) {
        ...
        try {
            for (i in 0..15) {
                ...
                // Section not set
                buf.writeChunkSection(section, ProtocolVersion.RELEASE_1_9_4)
